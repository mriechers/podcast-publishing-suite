...
Default behaviour:
  * Loads `knowledge/sources.json` if present.
  * Optionally prompts you to create or extend that list (`--init`, `--append`).
  * Fetches each source asynchronously, writing Markdown, HTML, and metadata files.

Example usage:
    python3.11 scripts/crawl_docs.py --init
    python3.11 scripts/crawl_docs.py --append --dry-run
    python3.11 scripts/crawl_docs.py --slug workflows --category alfred
"""
...
def prompt_for_sources(existing: List[dict]) -> List[dict]:
    print("Enter knowledge sources (leave category blank to finish):")
    ...
    while True:
        ...
        url = input("  URL: ").strip()
        if not url:
            print("    URL is required; skipping entry.")
            continue
...
def write_outputs(
    ...
    print(f"  -> {entry['category']}/{entry['slug']} (dry-run={dry_run})")
    if dry_run:
        return
    ...
    try:
        for entry in sources:
            ...
            try:
                ...
                write_outputs(base_path, entry, result.html or "", result.markdown.raw_markdown or "", metadata, dry_run)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sources", default="knowledge/sources.json", type=Path, help="JSON file describing knowledge sources.")
    ...
    parser.add_argument("--dry-run", action="store_true", help="Fetch pages but skip writing HTML/Markdown/JSON artifacts.")
    parser.add_argument("--init", action="store_true", help="Interactively define the sources list (overwrites existing list unless --append is set).")
    parser.add_argument("--append", action="store_true", help="Interactively add sources to the existing list.")
...
def main() -> int:
    ...
    if args.init and not args.append:
        ...
        print(f"No sources defined yet. Launching interactive setup for {args.sources}.")
        sources = prompt_for_sources(existing=[])
    ...
    if categories or slugs:
        sources_to_crawl = filter_sources(sources, categories, slugs)
    else:
        sources_to_crawl = sources

    succeeded, failed = asyncio.run(crawl_sources(sources_to_crawl, args.output, args.dry_run))
